...
<div class="form-horizontal">
	...
	<label for="datofalta" class="label-control col-md-3">
        ...
        <div class="col-md-9">
	 <g:select id="datofalta" name="datofalta.id" from="${catalogos.Campo.list()}" optionKey="id" required=""  noSelection="['':'']" class="form-control input-sm" value="${scaactInstance?.datofalta?.id}"   onchange="${remoteFunction(action:'getMostrar',params:'\'id=\'+ this.value',update:'modal')}" onBlur="${remoteFunction( action:'getEnvierro',params:'\'id=\'+ this.value',update:'envioerros')}"/>        
        </div> 
    </div>
<div id="modal"> </div>
...
<div class="fieldcontain ${hasErrors(bean: scaactInstance, field: 'expano', 'error')} required">
   ...
   <div class="col-md-3">   
        <g:select id="tipoerresp" name="tipoerresp.id" from="${catalogos.Erroresperado.list()}" optionKey="id"  value="2"/>           
